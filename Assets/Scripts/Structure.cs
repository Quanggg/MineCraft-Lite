...
public class Structure
{
  ...
  public static List<Change> Generate(Type type, int seed)
  {
    ...
    List<Change> temp = templates[type];
    List<Change> result = temp;
    switch (type)
    {
      case Type.OAK_TREE:
        result = new List<Change>();
        result.Add(new Change(0, -1, 0, BlockTypes.DIRT));
        bool cutOff = rand.Next(100) == 0;
        if (cutOff)
        {
          result.Add(new Change(0, 0, 0, BlockTypes.LOG_OAK));
          return result;
        }
        int height = (byte)rand.Next(4, 7);
        bool superHigh = rand.Next(100) == 0;
        ...
        if (rand.Next(0, 2) == 0) result.Add(new Change(1, height - 1, 1, BlockTypes.LEAVES_OAK));
        if (rand.Next(0, 2) == 0) result.Add(new Change(-1, height - 1, 1, BlockTypes.LEAVES_OAK));
        if (rand.Next(0, 2) == 0) result.Add(new Change(1, height - 1, -1, BlockTypes.LEAVES_OAK));
        if (rand.Next(0, 2) == 0) result.Add(new Change(-1, height - 1, -1, BlockTypes.LEAVES_OAK));
        ...
        for (int i = 2; i < 4; ++i)
        {
          ...
          if (rand.Next(0, 2) == 0) result.Add(new Change(2, height - i, 2, BlockTypes.LEAVES_OAK));
          if (rand.Next(0, 2) == 0) result.Add(new Change(-2, height - i, 2, BlockTypes.LEAVES_OAK));
          if (rand.Next(0, 2) == 0) result.Add(new Change(2, height - i, -2, BlockTypes.LEAVES_OAK));
          if (rand.Next(0, 2) == 0) result.Add(new Change(-2, height - i, -2, BlockTypes.LEAVES_OAK));
